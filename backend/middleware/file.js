...
const storage = multer.diskStorage({
  destination: (req, file, cb) => { // destination function dia akan jadi bila mulater nak save a file
    ...
    cb(error, "backend/images"); // klalau semua okay dia akn store file ikut alamt nie . and ikut penama kat bawah
  },
...
module.exports = multer({ storage: storage }).single("image"); /* pakai single sbb expect dpt single file. and multer akan carik image punye properties dalam request body which is nntie front bg */
//module.exports = multer({ storage: storage }).single("application");
